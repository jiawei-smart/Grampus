...
import org.grampus.swagger.config.model.Info;
import org.grampus.swagger.config.model.Scheme;
...
public class GSwagger {
    private final Swagger swagger;
    private final Service spark;
    private final GSwaggerOptions options;
    private final String apiPath;

    private GSwagger(final Service spark, final GSwaggerOptions options) {
        this.spark = spark;
    ...
    public static GSwagger of(final Service spark) {
        return new GSwagger(spark, GSwaggerOptions.defaultOptions());
    }

    public static GSwagger of(final Service spark, final GSwaggerOptions options) {
        Objects.requireNonNull(options);
        return new GSwagger(spark, options);
    }
    ...
    public ApiEndpoint endpoint(final EndpointSpec.Builder descriptorBuilder) {
        return endpoint(descriptorBuilder, null);
    }

    public ApiEndpoint endpoint(final EndpointSpec.Builder descriptorBuilder, final Filter filter) {
        ...
        EndpointSpec descriptor = descriptorBuilder.build();
    ...
    public GSwagger endpoint(final EndpointSpec.Builder descriptorBuilder, final Filter filter, Consumer<ApiEndpoint> endpointDef) {
        Optional.ofNullable(apiPath).orElseThrow(() -> new IllegalStateException("API Path must be specified in order to build REST endpoint"));
        EndpointSpec descriptor = descriptorBuilder.build();
    ...
    public GSwagger endpoint(final Endpoint endpoint) {
        Optional.ofNullable(endpoint).orElseThrow(() -> new IllegalStateException("API Endpoint cannot be null"));
        endpoint.bind(this);
        return this;
    }

    public GSwagger endpoints(final Supplier<List<Endpoint>> resolver) {
    ...
    public GSwagger before(Filter filter) {
        spark.before(apiPath + "/*", filter);
        return this;
    }

    public GSwagger after(Filter filter) {
        spark.after(apiPath + "/*", filter);
        return this;
    }

    public synchronized GSwagger exception(Class<? extends Exception> exceptionClass, final ExceptionHandler handler) {
...
//        parseYaml(swagger, uiFolder + "doc.yaml");
    ...
    private void extractUi(String uiFolder) throws IOException {
        CodeSource src = GSwagger.class.getProtectionDomain().getCodeSource();
        if (src != null) {
